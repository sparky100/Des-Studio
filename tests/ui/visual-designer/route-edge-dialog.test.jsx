...
// Single, not-yet-consolidated destination — no routing/probabilisticRouting array —
// the shape connectVisualNodes still produces for an activity's very first connection.
function makeSingleRouteModel() {
  return {
    entityTypes: [
      { id: 'customer-1', name: 'Customer', role: 'customer' },
      { id: 'server-1', name: 'Server', role: 'server', count: 1 },
    ...
    bEvents: [
      { id: 'arrival-1', name: 'Arrivals', scheduledTime: '0', effect: 'ARRIVE(Customer, Queue 1)',
        schedules: [{ eventId: 'arrival-1', dist: 'Exponential', distParams: { mean: '5' } }] },
      { id: 'route-activity-1-queue-2', name: 'Triage Complete', scheduledTime: '9999', effect: 'RELEASE(Server, Queue 2)', schedules: [] },
    ],
    cEvents: [{
      id: 'activity-1',
      name: 'Triage',
...
function setup(model = makeModel(), overrides = {}) {
  ...
  const edge = graph.edges.find(e => e.bEventId === 'route-activity-1-queue-2' && e.branchIndex === 0)
    || graph.edges.find(e => e.from === 'activity:activity-1' && e.source === 'routing');
  const onApply = vi.fn();
  const onClose = vi.fn();
  ...
  return { onApply, onClose, onDeleteEdge, edge };
}
...
// Deletion for an activity's route is handled entirely inside this dialog (the
// canvas's inline delete button is suppressed for these edges — see
// FlowDiagramReactFlow's isActivityRouteEdge gating) rather than split across two
// competing affordances. The "none" (single-destination, not-yet-split) mode has
// no per-branch row to remove, so it needs its own explicit delete action.
describe('RouteEdgeDialog — deleting a single-destination (mode="none") route', () => {
  it('shows a Delete connection button next to Done when there is only one destination', () => {
    setup(makeSingleRouteModel());
    expect(screen.getByRole('button', { name: /delete connection/i })).toBeInTheDocument();
  });

  it('clicking Delete connection calls onDeleteEdge with the edge id', () => {
    const { onDeleteEdge, edge } = setup(makeSingleRouteModel());
    fireEvent.click(screen.getByRole('button', { name: /delete connection/i }));
    expect(onDeleteEdge).toHaveBeenCalledWith(edge.id);
  });

  it('hides the Delete connection button when canEdit is false', () => {
    setup(makeSingleRouteModel(), { canEdit: false });
    expect(screen.queryByRole('button', { name: /delete connection/i })).not.toBeInTheDocument();
  });

  it('does not show a Delete connection button once the route has multiple branches (removal happens per-row instead)', () => {
    setup(makeModel());
    expect(screen.queryByRole('button', { name: /delete connection/i })).not.toBeInTheDocument();
  });
});
