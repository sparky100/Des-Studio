...
export function buildModelBuilderSystemPrompt() {
  ...
  return [
    ...
    "Phase A — Discover: If the user's request is missing genuinely critical structural information (e.g. no indication of entity type, no hint of server count), ask AT MOST 2 focused questions in a single turn. Use intent: clarify. If you can make a reasonable assumption (e.g. Exponential inter-arrivals, Fixed service time, a single queue), make it and note it in Phase B rather than asking. Do not ask about numeric values you can default — only ask about structure that would materially change the model shape. For well-known scenarios (post office, bank, clinic, call centre) you almost always have enough to proceed straight to Phase B.",
    "Phase B — Confirm: Once you have enough information (or can fill gaps with sensible defaults), describe in plain English exactly what you are about to build: entity types, arrival pattern with assumed values, queues, service stages, server count, and experiment duration. Return intent: confirm with this summary as explanation. Do NOT include proposedModel. Wait for the user to confirm before proceeding to Phase C.",
    "Phase C — Generate: After the user confirms (any affirmative — 'yes', 'looks good', 'build it', 'correct', 'go ahead', etc.), generate the complete JSON model. Return intent: build or refine with proposedModel populated.",
...
export function buildModelBuilderUserMessage(description, currentModel = {}, results = null) {
  ...
  const instruction = hasCurrentModel
    ...
    : "Follow the three-phase conversation discipline from the system prompt. For well-known scenarios (post office, bank, clinic, call centre, etc.) or requests that include enough structural detail, skip Phase A and go straight to Phase B: return intent: confirm with a plain-English summary of what you will build (entity types, arrival rate assumption, server count, service time assumption). Only use Phase A (intent: clarify) if critical structure is genuinely ambiguous — and then ask at most 2 questions. Do NOT generate proposedModel until the user confirms.";
