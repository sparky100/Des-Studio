...
export function buildModelBuilderSystemPrompt() {
  ...
  return [
    ...
    "PHASE A — Discover: Ask ONE question at a time. No fixed cap on questions, but each must be purposeful — targeted at information that materially affects model structure. Follow this question order, stopping as soon as you have enough to describe the full system:",
    "  1. What flows through the system? (defines customer entity type)",
    "  2. How do they arrive — continuously, in batches, or on a schedule? (drives B-event distribution choice)",
    ...
    "  5. How long does each stage typically take, and how variable is it? (service distribution type and parameters)",
    ...
    "After each user answer: ask the next question (intent: clarify, questions: [\"...\"], proposedModel: null), OR move to PHASE B when you can describe the full system without ambiguity. For well-known scenarios (post office, bank, clinic, call centre) you almost always have enough to proceed straight to PHASE B after one or two questions.",
    "PHASE B — Confirm: When you have enough information, do NOT generate JSON. Return intent: confirm, questions: null, proposedModel: null. The explanation field MUST follow this exact structure:",
    "  \"Here is my understanding of your system:\\n\\nArrivals: [entity type name] arrive every [value] [time unit] on average ([distribution name] distribution)\\n\\nFlow: [stage 1 queue name] → [stage 1 server type, N servers, service distribution] → [stage 2 if present] → exit\\n[Reneging: entities abandon after ~[N] [timeUnit] if this was mentioned]\\n\\nQueue discipline: [FIFO / PRIORITY / LIFO] at [stage name]\\n\\nGoal: [performance target if stated, or 'not specified']\\n\\nDoes this accurately describe your system? I'll build the model once you confirm, or correct anything that looks wrong.\"",
    "PHASE C — Generate: Triggered only when the user replies affirmatively ('yes', 'correct', 'looks right', 'go ahead', 'build it'). Generate: intent: build, proposedModel: complete model JSON, explanation: 3–5 sentence plain-English summary, suggestions: array of exactly 3 short refinement question strings.",
    "Never skip PHASE B for a new build request. Always show the plain-English summary and wait for confirmation before generating JSON. For refine requests where the change is a simple addition (e.g. 'add a second server'), PHASE B is optional.",
...
export function buildModelBuilderUserMessage(description, currentModel = {}, results = null) {
  ...
  const instruction = hasCurrentModel
    ...
    : "Follow the three-phase conversation discipline from the system prompt. For well-known scenarios (post office, bank, clinic, call centre, etc.) or requests that include enough structural detail, skip PHASE A and go straight to PHASE B: return intent: confirm with a plain-English summary using the required format. Only use PHASE A (intent: clarify) if critical structure is genuinely ambiguous — and then ask ONE question at a time. Do NOT generate proposedModel until the user confirms.";
