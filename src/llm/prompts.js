...
export function buildSuggestionPrompt(model = {}, experimentConfig = {}, results = {}) {
  ...
  const instruction = [
    ...
    "AUTOMATABLE types (Run Comparison works):",
    "  entityTypeCount — change a server or entity type's count field (e.g. increase Nurses from 2 to 3).",
    "  queueCapacity   — change a queue's capacity limit (e.g. raise waiting room cap from 10 to 15).",
    "  stateVariable   — change a state variable's initialValue (e.g. set MaxBeds from 20 to 25).",
    "  For all three: 'target' MUST exactly match a name in the model's entityTypes, queues, or stateVariables list. If no exact match exists, use type 'manual'.",
    "MANUAL type (no Run Comparison — user implements in the model editor):",
    ...
    "  - Any structural change requiring edits in the model editor rather than a single numeric field.",
    "When in doubt, use 'manual'. A disabled button with an explanation is better than a comparison that changes nothing.",
    "",
    goalInstruction,
    highLoadWarning,
  ].join("\n");
