...
export function buildExplainResultsPrompt(model = {}, experimentConfig = {}, results = {}, ciResults = []) {
  ...
  const instruction = [
    ...
    wipInstr ? "IF THE RUN ENDED WITH UNFINISHED ENTITIES: The terminatingState data shows entities that never completed their journey. This is a reliability issue — average service times, wait times, and throughput may all be understated because long-running entities remain in-progress at termination. Call this out as a caveat in the 'How Reliable' section before discussing specific metrics." : "",
    "",
    "## What to Change",
