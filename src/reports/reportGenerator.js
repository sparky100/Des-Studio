...
function buildCover(model, runMeta, experimentConfig) {
  ...
  <div class="cover">
    ...
    <div class="meta">
      ...
      ${periodLine}
      <div><strong>Engine:</strong> flow v${esc(runMeta.engineVersion || '1.0')}</div>
    </div>
...
function buildRunProvenance(runMeta) {
  ...
  <section>
    ...
    <p class="note">This report was generated from a run record stored in flow. The model definition, experiment configuration, and results are preserved in the run record and can be reproduced exactly using the Reproduce Run function.</p>
  </section>`;
}
...
<div class="report">
  ${body}
  <div class="footer">Generated by flow · ${esc(formatDate(new Date().toISOString()))}</div>
</div>
</body>
</html>`;
}
...
function buildMarkdownReport({ model, results, experimentConfig, runMeta, aggregateStats, type, narrativeText, modelDescription, recommendations }) {
  ...
  lines.push('---');
  lines.push('');
  lines.push(`_Generated by flow · ${formatDate(new Date().toISOString())}_`);
...
<body>
  ...
  ${sections}
  <div class="footer">Generated by flow</div>
</body>
</html>`;
}
