...
describe("CHK-008 Server defined but never used in C-event", () => {
  ...
  test("does not trigger for a server only referenced via a skill-qualified ASSIGN (quoted skill literal)", () => {
    const model = {
      ...
      entityTypes: [
        ...wellFormedModel.entityTypes,
        { id: "et3", name: "Pathologist", role: "server", count: "1", attrDefs: [] },
      ],
      cEvents: [
        ...wellFormedModel.cEvents,
        {
          ...
          priority: 1,
          condition: { clauses: [{ variable: "Queue.ServiceQueue.length", operator: ">", value: 0 }], logic: "AND" },
          effect: ['ASSIGN(ServiceQueue, Pathologist, "Histopathology")'],
          cSchedules: [],
        },
      ],
    };
  ...
  test("does not trigger for a server only referenced via a skill-qualified ASSIGN (Entity.attr form)", () => {
    const model = {
      ...wellFormedModel,
      entityTypes: [
        ...wellFormedModel.entityTypes,
      ...
      cEvents: [
        ...wellFormedModel.cEvents,
        {
          ...
          priority: 1,
          condition: { clauses: [{ variable: "Queue.ServiceQueue.length", operator: ">", value: 0 }], logic: "AND" },
          effect: ["ASSIGN(ServiceQueue, Pathologist, Entity.requiredSkill)"],
          cSchedules: [],
        },
      ],
    };
