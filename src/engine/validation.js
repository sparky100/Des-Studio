    ...
    (b.schedules || []).forEach((/** @type {any} */ s, /** @type {any} */ j) => {
      ...
      if (distName === 'Exponential') {
        ...
        if (Number.isFinite(mean) && mean < 0.001) {
          warn('W-CAP-02',
            `B-Event '${b.name || b.id}' schedule ${j + 1}: Exponential mean interval = ${mean} (< 0.001). flow models discrete individual entities. For continuous flow or aggregate quantities, consider SD Studio.`,
            'bevents',
            { eventIds: [b.id] });
        }
      }
