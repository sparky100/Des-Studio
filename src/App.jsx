...
export default function App(){
  ...
  useEffect(()=>{
    try{
      ...
      setAppError('Auth setup error: '+e.message);
      setLoading(false);
    }
  },[]);
  ...
  const loadData=useCallback(async()=>{
    ...
    }catch(e){setAppError('Load error: '+e.message);}
    finally{setLoading(false);}
  },[session]);

  useEffect(()=>{loadData();},[loadData]);

  // Show any top-level error on screen
  if(appError)return(
    <div style={{background:C.bg,minHeight:'100vh',display:'flex',alignItems:'center',
      justifyContent:'center',color:C.red,fontFamily:'monospace',fontSize:14,padding:24,textAlign:'center'}}>
      <div>
  ...
  if(loading)return(
    <div style={{background:C.bg,minHeight:'100vh',display:'flex',alignItems:'center',
      justifyContent:'center',color:C.muted,fontFamily:FONT,fontSize:13}}>
    ...
    return(
      <div style={{background:C.bg,minHeight:'100vh'}}>
        ...
        <ModelDetail modelId={openId}
          ...
          onRefresh={loadData}
          overrides={{isOwner,canEdit,profiles,
