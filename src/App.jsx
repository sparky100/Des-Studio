...
const createDB = () => {
  const USERS = {
    u1:{id:"u1",name:"Alex Morgan", role:"Analyst",   initials:"AM",color:"#06b6d4"},
    u2:{id:"u2",name:"Jordan Blake",role:"Planner",   initials:"JB",color:"#f59e0b"},
    u3:{id:"u3",name:"Sam Chen",    role:"Researcher",initials:"SC",color:"#8b5cf6"},
  };
  const seed = {
    m1:{
      id:"m1", name:"Queue with Reneging — 2 Servers",
      description:"Customers arrive (Exponential). Two servers. Impatient customers renege if not served in time. C-event assigns customer to idle server then schedules completion using server.serviceTime attribute.",
      owner:"u1", visibility:"public", access:{u2:"viewer"},
      entityTypes:[
        {id:"et1",name:"Customer",role:"customer",count:"",attrs:"",description:"Arriving customer"},
        {id:"et2",name:"Server",  role:"server",  count:"2",attrs:"serviceTime=3",description:"Two servers, serviceTime=3"},
      ],
      stateVariables:[{id:"sv1",name:"totalArrived",initialValue:"0",description:"Cumulative arrivals"}],
      bEvents:[
        {id:"b1",name:"Customer Arrives",scheduledTime:"0",
          effect:"ARRIVE(Customer); totalArrived++",
          schedules:[
            {eventId:"b1",dist:"Exponential",distParams:{mean:"2"},isRenege:false},
            {eventId:"b2",dist:"Uniform",distParams:{min:"5",max:"12"},isRenege:true},
          ],
          description:"Customer arrives. Schedules next arrival + reneging timer."},
        {id:"b2",name:"Customer Reneges",scheduledTime:"999",
          effect:"RENEGE(ctx)",schedules:[],description:"Patience expires — skip if already served."},
        {id:"b3",name:"Service Complete",scheduledTime:"999",
          effect:"COMPLETE()",schedules:[],description:"Server finishes — scheduled by C-event."},
      ],
      cEvents:[
        {id:"c1",name:"Start Service",
          condition:"queue(Customer).length > 0 AND idle(Server).count > 0",
          effect:"ASSIGN(Customer, Server)",
          cSchedules:[{eventId:"b3",dist:"ServerAttr",distParams:{attr:"serviceTime"},useEntityCtx:true}],
          description:"Assign customer to idle server. Schedule Service Complete using server.serviceTime attribute."},
      ],
      stats:null,createdAt:"2026-04-22T09:00:00Z",updatedAt:"2026-04-22T10:00:00Z",
    },
    m2:{
      id:"m2", name:"Fast & Slow Servers",
      description:"One fast server (serviceTime=1.5) and one slow server (serviceTime=4). Demonstrates attribute-driven service duration with two distinct server types.",
      owner:"u1", visibility:"public", access:{},
      entityTypes:[
        {id:"et1",name:"Customer",   role:"customer",count:"",  attrs:"",description:"Standard customer"},
        {id:"et2",name:"FastServer", role:"server",  count:"1", attrs:"serviceTime=1.5",description:"Fast server"},
        {id:"et3",name:"SlowServer", role:"server",  count:"1", attrs:"serviceTime=4",  description:"Slow server"},
      ],
      stateVariables:[{id:"sv1",name:"totalArrived",initialValue:"0",description:"Cumulative arrivals"}],
      bEvents:[
        {id:"b1",name:"Customer Arrives",scheduledTime:"0",
          effect:"ARRIVE(Customer); totalArrived++",
          schedules:[{eventId:"b1",dist:"Exponential",distParams:{mean:"1.8"}}],
          description:"Customer arrives. Next: Exponential(1.8)."},
        {id:"b2",name:"Service Complete",scheduledTime:"999",
          effect:"COMPLETE()",schedules:[],description:"Service complete — scheduled by C-events."},
      ],
      cEvents:[
        {id:"c1",name:"Assign to Fast Server",
          condition:"queue(Customer).length > 0 AND idle(FastServer).count > 0",
          effect:"ASSIGN(Customer, FastServer)",
          cSchedules:[{eventId:"b2",dist:"Normal",distParams:{mean:"1.5",stddev:"0.3"},useEntityCtx:true}],
          description:"Prefer fast server. Service Complete: Normal(1.5, 0.3)."},
        {id:"c2",name:"Assign to Slow Server",
          condition:"queue(Customer).length > 0 AND idle(SlowServer).count > 0",
          effect:"ASSIGN(Customer, SlowServer)",
          cSchedules:[{eventId:"b2",dist:"Normal",distParams:{mean:"4",stddev:"0.8"},useEntityCtx:true}],
          description:"Use slow server when fast is busy. Service Complete: Normal(4, 0.8)."},
      ],
      stats:null,createdAt:"2026-04-22T09:01:00Z",updatedAt:"2026-04-22T10:00:00Z",
    },
  };

  let _store=seed, _current="u1", _nextId=10;
  const ts=()=>new Date().toISOString();
  return {
    currentUser:()=>USERS[_current], allUsers:()=>Object.values(USERS),
    getUser:id=>USERS[id], switchUser:id=>{_current=id;},
    myModels:()=>Object.values(_store).filter(m=>m.owner===_current||m.access?.[_current]),
    publicModels:()=>Object.values(_store).filter(m=>m.visibility==="public"),
    get:id=>_store[id]||null, isOwner:id=>_store[id]?.owner===_current,
    canEdit:id=>{const m=_store[id];return m&&(m.owner===_current||m.access?.[_current]==="editor");},
    save:data=>{
      if(data.id&&_store[data.id]){_store[data.id]={..._store[data.id],...data,updatedAt:ts()};return _store[data.id];}
      const id="m"+(++_nextId);
      const m={...data,id,owner:_current,stats:null,stateVariables:[],entityTypes:[],bEvents:[],cEvents:[],
        createdAt:ts(),updatedAt:ts(),access:{},visibility:"private"};
      _store[id]=m;return m;},
    delete:id=>{delete _store[id];},
    setVisibility:(id,vis)=>{if(_store[id]?.owner===_current){_store[id].visibility=vis;_store[id].updatedAt=ts();}},
    setAccess:(mid,uid,role)=>{if(!_store[mid])return;if(role==="none")delete _store[mid].access[uid];else _store[mid].access[uid]=role;_store[mid].updatedAt=ts();},
  };
};
const DB=createDB();
...
export default function App() {
  ...
  const [cu,setCu]=useState(DB.currentUser());
  const refresh=()=>setTick(t=>t+1);
  const switchUser=id=>{DB.switchUser(id);setCu(DB.currentUser());setOpenId(null);refresh();};
  const myModels=DB.myModels();
  const publicModels=DB.publicModels().filter(m=>m.owner!==cu.id);
  ...
  if(openId)return(
    <div style={{background:C.bg,minHeight:"100vh"}}>
      ...
      <ModelDetail modelId={openId} onBack={()=>{setOpenId(null);refresh();}} onRefresh={refresh}/>
    </div>
  );
  return (
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:FONT}}>
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 24px",display:"flex",alignItems:"center",gap:16,height:52}}>
        ...
        <div style={{flex:1}}/>
        <span style={{fontSize:10,color:C.muted}}>USER:</span>
        {DB.allUsers().map(u=>(
          <button key={u.id} onClick={()=>switchUser(u.id)} style={{background:cu.id===u.id?u.color+"22":"transparent",border:`1px solid ${cu.id===u.id?u.color+"66":C.border}`,borderRadius:5,padding:"4px 10px",cursor:"pointer",color:cu.id===u.id?u.color:C.muted,fontFamily:FONT,fontSize:11,fontWeight:600}}>{u.initials}</button>
        ))}
      </div>
      <div style={{maxWidth:1100,margin:"0 auto",padding:"28px 24px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
          <div>
            ...
            <p style={{fontSize:12,color:C.muted}}>Three-Phase DES · Entity tracking · Server attributes · Step-through execution · Visual view</p>
          </div>
        ...
        {tab==="my"&&(myModels.length===0?<Empty icon="📐" msg="No models yet."/>:
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
            {myModels.map(m=><ModelCard key={m.id+tick} model={m} onOpen={()=>setOpenId(m.id)}/>)}
          </div>)}
        {tab==="public"&&(publicModels.length===0?<Empty icon="🌐" msg="No public models."/>:
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
            {publicModels.map(m=><ModelCard key={m.id+tick} model={m} onOpen={()=>setOpenId(m.id)}/>)}
          </div>)}
      </div>
      {showNew&&<NewModelModal onClose={()=>setShowNew(false)} onCreate={id=>{refresh();setOpenId(id);}}/>}
    </div>
  );
}
