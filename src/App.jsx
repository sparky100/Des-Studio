  ...
  const submit=async()=>{
    ...
    try{
      if(mode==='login'){
        ...
        if(error)throw error;
        setInfo('Account created! Check your email or sign in directly.');
        setMode('login');
      }
      ...
      <div style={{width:380,display:'flex',flexDirection:'column',gap:20}}>
        ...
        <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:10,
          ...
          <div style={{display:'flex',borderBottom:`1px solid ${C.border}`,marginBottom:4}}>
            {['login','signup'].map(m=>(
              <button key={m} onClick={()=>setMode(m)} style={{flex:1,background:'none',border:'none',
                borderBottom:mode===m?`2px solid ${C.accent}`:'2px solid transparent',
                color:mode===m?C.accent:C.muted,fontFamily:FONT,fontSize:12,padding:'8px 0',
                cursor:'pointer',fontWeight:mode===m?700:400,textTransform:'uppercase',letterSpacing:1}}>
          ...
          {mode==='signup'&&(
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              <label style={{fontSize:10,fontWeight:700,letterSpacing:1.5,color:C.muted,textTransform:'uppercase'}}>Full Name</label>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:5}}>
            <label style={{fontSize:10,fontWeight:700,letterSpacing:1.5,color:C.muted,textTransform:'uppercase'}}>Password</label>
            <input value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" type="password" style={inp}/>
          </div>
          {error&&<div style={{background:C.red+'18',border:`1px solid ${C.red}44`,
            borderRadius:5,padding:'8px 12px',fontSize:12,color:C.red}}>{error}</div>}
          {info&&<div style={{background:C.green+'18',border:`1px solid ${C.green}44`,
            borderRadius:5,padding:'8px 12px',fontSize:12,color:C.green}}>{info}</div>}
          <button onClick={submit} disabled={loading} style={{background:C.accent,color:'#080c10',
            border:'none',borderRadius:6,padding:'11px 0',fontFamily:FONT,fontSize:13,fontWeight:700,
          ...
          <div style={{maxWidth:480,display:"flex",flexDirection:"column",gap:14}}>
            ...
            {(overrides.profiles||DB.allUsers().map(u=>({id:u.id,full_name:u.name,initials:u.initials,color:u.color}))).filter(u=>u.id!==model.owner_id&&u.id!==model.owner).map(u=>(
              <div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                ...
                <select value={model.access?.[u.id]||"none"} onChange={e=>{const newAccess={...(model.access||{}),[u.id]:e.target.value};if(overrides.onSetAccess)overrides.onSetAccess(modelId,newAccess).then(onRefresh);else{DB.setAccess(modelId,u.id,e.target.value);setModel({...DB.get(modelId)});}}}
                  style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:4,color:C.text,fontFamily:FONT,fontSize:11,padding:"4px 8px",outline:"none"}}>
...
export default function App(){
  ...
  if(loading)return(
    <div style={{background:C.bg,minHeight:'100vh',display:'flex',
      alignItems:'center',justifyContent:'center',color:C.muted,fontFamily:FONT,fontSize:13}}>
    ...
    <div style={{background:C.bg,minHeight:'100vh',color:C.text,fontFamily:FONT}}>
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,
        ...
        <div style={{fontSize:11,color:C.muted,borderLeft:`1px solid ${C.border}`,paddingLeft:16}}>
          Three-Phase · Entities · Servers · Step-Through
        </div>
        <div style={{flex:1}}/>
        {profile&&(
        ...
        <button onClick={signOut} style={{background:'#ffffff08',border:`1px solid ${C.border}`,
          borderRadius:5,color:C.muted,fontFamily:FONT,fontSize:11,
          padding:'5px 12px',cursor:'pointer',fontWeight:600}}>Sign Out</button>
      </div>

      <div style={{maxWidth:1100,margin:'0 auto',padding:'28px 24px'}}>
        {error&&<div style={{background:C.red+'18',border:`1px solid ${C.red}44`,
          borderRadius:6,padding:'10px 14px',fontSize:12,color:C.red,
          fontFamily:FONT,marginBottom:16}}>{error}</div>}
        ...
        <div style={{display:'flex',borderBottom:`1px solid ${C.border}`,marginBottom:24}}>
          {[
            ...
            <button key={t.id} onClick={()=>setTab(t.id)} style={{background:'none',border:'none',
              ...
              padding:'10px 18px',cursor:'pointer',fontWeight:tab===t.id?700:400}}>{t.label}</button>
          ))}
        </div>
