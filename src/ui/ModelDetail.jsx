  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100dvh",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"clamp(12px,2vw,20px)"}}>
        ...
        {tab==="queues"&&renderAuthoringShell(<div style={{maxWidth:920,margin:"0 auto"}}><TabErrors tabId="queues" validation={validation} onErrorClick={({tab,affectedIds})=>setErrorFilter({tab,affectedEventIds:affectedIds?.eventIds,affectedQueueIds:affectedIds?.queueIds,affectedEntityTypeIds:affectedIds?.entityTypeIds})}/><QueueEditor queues={model.queues||[]} entityTypes={model.entityTypes||[]} stateVariables={model.stateVariables||[]} sections={model.sections||[]} containerTypes={model.containerTypes||[]} errorFilter={errorFilter?.tab==="queues"?{filteredQueueIds:errorFilter.affectedQueueIds}:null} onClearErrorFilter={()=>setErrorFilter(null)} onChange={canEdit?newQueues=>{
