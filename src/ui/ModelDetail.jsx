...
import { Tag, Btn, Field, SH, InfoBox, Empty, ErrorBoundary } from "./shared/components.jsx";
...
const ModelDetail=({modelId,modelData,onBack,onRefresh,onLatestVersionChange,overrides={},initialTab})=>{
  ...
  // Visibility changes persist immediately (same contract as persistAccess):
  // optimistic flip, then the DB write, surfacing failures as a toast.
  const handleChangeVisibility=(vis)=>{
    setModel(m=>({...m,visibility:vis}));
    if(overrides.onSetVisibility)overrides.onSetVisibility(modelId,vis).then(onRefresh).catch(err=>toast.error(err?.message||"Could not update visibility"));
  };
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100dvh",background:C.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"clamp(12px,2vw,20px)"}}>
        ...
        {tab==="schedules"&&renderAuthoringShell(<div style={{maxWidth:1120,margin:"0 auto",display:"flex",flexDirection:"column",gap:24}}>
        ...
        {tab==="access"&&isOwner&&(
          <AccessTab
            ...
            profiles={overrides.profiles} userId={overrides.userId}
            onChangeVisibility={handleChangeVisibility}
            onPersistAccess={persistAccess}
            onExportJson={exportJson}
            onExportSimPy={()=>setShowSimPyExport(true)}
            setField={setField} dirty={dirty}
          />
        )}
