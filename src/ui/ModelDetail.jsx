    ...
    <div style={{display:"flex",flexDirection:"column",height:"100dvh",minHeight:"100vh",background:C.bg}}>
      ...
      <ModelTabBar
        ...
        onPrintDiagram={async () => {
          ...
          win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>${e(model.name||"Model")} — Diagram</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:"Segoe UI",Arial,sans-serif;font-size:13px;color:#1a1a1a;background:#fff;padding:32px 40px;max-width:1200px;margin:0 auto}h1{font-size:22px;font-weight:700;margin-bottom:4px}.sub{color:#555;font-size:12px;margin-bottom:8px}.desc{color:#333;font-size:13px;margin-bottom:20px;line-height:1.6}img{max-width:100%;border:1px solid #eee}.note{color:#888;font-style:italic;font-size:12px}.footer{margin-top:32px;font-size:11px;color:#888;border-top:1px solid #eee;padding-top:10px}@media print{body{padding:0}@page{margin:1.5cm 1.8cm}}</style></head><body><h1>${e(model.name||"Model")}</h1><div class="sub">Diagram · Printed ${e(printDate)}</div>${model.description?`<div class="desc">${e(model.description)}</div>`:""}<div style="margin-top:16px">${dataUrl?`<img src="${dataUrl}" alt="Model diagram" />`:`<p class="note">Diagram image could not be captured. Please open the Draw view and try again.</p>`}</div><div class="footer">Generated by simmodlr</div></body></html>`);
