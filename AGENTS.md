...
| `soak` | `node`, `maxForks: 2` | full-simulation / replication / analytical-benchmark files (see below) | 240s / 240s hooks |

`npm test` runs `unit` + `ui` only (the fast tier, real cross-file parallelism via Vitest's default `forks` pool). `npm run test:soak` runs the heavy tier. Pool-level settings (`maxForks: 2`, and a bumped `execArgv` heap ceiling — `--max-old-space-size=6144`) live on the **root** `test` block in `vite.config.js`, not inside any individual project: `poolOptions` is a pool-construction-time setting shared by all projects using the same pool, so a value set inside a project's own `test` block is silently ignored (see §22.1b for how this was discovered, and don't repeat the mistake). `maxForks: 2` bounds how many full simulations run concurrently (protects `performance.test.js`'s wall-clock assertion from CPU contention) without forcing the whole run into one process (that reintroduces the OOM described in §22.1b: `singleFork` disables Vitest's per-file worker-process recycling, so heap from earlier heavy files accumulates for the rest of the run); the heap bump covers `determinism-parity.test.js`'s worst-case single-file footprint, which exceeds Node's ~4GB default on its own. `npm run test:all` runs everything and is what CI's overall bar amounts to (`test` and `soak` are separate, parallel CI jobs, both blocking).
...
export default defineConfig({
  test: {
    ...
    // Pool-construction-time settings — MUST live here (root), not inside
    // any project below. See §22.1b.
    poolOptions: { forks: { maxForks: 2, execArgv: ['--max-old-space-size=6144'] } },
    projects: [
      ...
      { extends: true, test: { name: 'soak', environment: 'node', setupFiles: ['tests/setup-node.js'], testTimeout: 240000, hookTimeout: 240000, include: [...] } },
    ],
  },
});
```
...
- `results_json.summary.outcomes` records journey conclusions by terminal route (`routeId`, `routeLabel`, `status`, `endedBy`, `count`). Entity snapshots may also include `entity.outcome` with the same route metadata. Reports, exports, Results, and LLM prompts should use this rather than inferring route-specific completion counts from logs.
...
The `soak` project's first version set `singleFork: true` (reasoning: keep heavy simulations from competing with each other for CPU) — this reintroduced the exact same class of bug at a smaller scale: `singleFork` disables Vitest's per-file worker-process recycling (its `isolate: true` default normally resets the heap between files by restarting the process), so heap from `soak`'s earlier heavy files accumulated across its whole run until a later file (the ~100s `refugee-displacement-corridor` scenario) tipped it over the ceiling — reproduced in CI, not locally, which has more headroom. The attempted fix was capping concurrency instead (`maxForks: 2`) — correct in principle, but placed in the `soak` project's own `test` block, which turned out not to work (see below).

**Two more rounds, both discovered only by pushing to CI (neither reproduced locally beforehand):**

1. `poolOptions` (`maxForks`, `execArgv`, `singleFork`, `isolate`) is a **pool-construction-time** setting, not a per-file/per-test one. `test.projects` all share ONE 'forks' pool instance, built once from Vitest's root config (confirmed by reading `createForksPool()`/`createPool()` in `vitest/dist/chunks/coverage.*.js`, and empirically: an absurd `execArgv: ['--max-old-space-size=64']` nested inside `soak`'s `test` block had zero effect, while the identical value at the root `test` block crashed the worker almost instantly at ~64MB, as expected). So `maxForks: 2` set inside the `soak` project was silently ignored — the pool kept using Vitest's CPU-count-based default, which is why files still ran in a way that superficially looked capped-but-not-quite. **Fix:** moved `poolOptions` to the root `test` block (`vite.config.js`), where it applies to all three projects (harmless for `unit`/`ui` — more heap headroom never hurts, and `maxForks: 2` still leaves real parallelism, just bounded rather than unbounded).

2. Once `maxForks: 2` genuinely took effect, `tests/benchmarks/golden.test.js`'s M/M/c `beforeAll` started silently skipping its two dependent tests (a legitimate CI-only failure mode, exit code 0-looking but actually a hook timeout, not caught by "0 failures" style checks). Root cause: Vitest's `beforeAll`/`afterAll` hooks use a **separate** default timeout (`hookTimeout`, 10s) from `testTimeout` — not obvious, since a hook that just wraps a `run()` call reads like it inherits the test's own budget. `testTimeout: 240000` on the `soak` project never applied to this hook; the fixture's `beforeAll` (added when its two tests were consolidated from separate `{ timeout: 30000 }` tests) carried an explicit `30000` — a *test*-timeout value, reused as a hook-timeout argument, still the wrong kind of budget even though the number looked reasonable for a ~19-20s run. **Fix:** `hookTimeout: 240000` added to the `soak` project, matching `testTimeout`, so this class of bug can't recur for any future `soak`-tier `beforeAll`/`afterAll`.

The general lesson for anyone touching `vite.config.js`'s `test` block: **pool-level settings (`poolOptions`, `pool`) belong on the root `test` object; everything else (`testTimeout`, `hookTimeout`, `include`/`exclude`, `environment`, `setupFiles`) belongs on the individual project**. Getting this backwards fails silently — no error, no warning, just the wrong (usually default) behavior — so verify empirically (an absurd value that should obviously crash or obviously pass) rather than trusting that a value "looks like" it's in the right place.
