...
| **Distance** | A named, undirected travel distance between two queues, set in the **Reference Data** tab. Consumed by picking **Distance** as a C-event schedule's delay distribution, which computes duration as distance ÷ a speed attribute read from the assigned server or arriving entity. |
| **Reference Data (tab)** | The Model Detail tab holding small named lookup-table registries that other parts of the model reference by name — currently Containers (tanks/buffers) and Distances (travel time between queues). |
