# DES Studio — Competitive Comparison & Gap Assessment 2026
...
### DES Studio Positioning

DES Studio occupies a previously empty niche: **browser-native, no-install, AI-augmented, collaboratively shareable DES** with structured (non-code) model authoring. It competes on accessibility and modern workflow rather than raw simulation power. The nearest direct competitor is **JaamSim** (visual, desktop, open-source) — DES Studio matches or exceeds JaamSim on statistical output, AI integration, and sharing, while trailing on 3D visualisation and scale.
...
| Feature | SimPy | salabim | JaamSim | simmer | AnyLogic | Simio | Arena | FlexSim | **DES Studio** | Gap? |
|---------|-------|---------|---------|--------|----------|-------|-------|---------|---------------|------|
...
*Many features in this category are DES Studio innovations without direct comparator equivalents.*
...
### Where DES Studio Leads (Differentiators)

| Differentiator | DES Studio | Nearest Competitor | Notes |
|---------------|-----------|-------------------|-------|
| Browser-native, zero install | ✅ | ❌ None | All professional and open-source tools require installation. DES Studio runs in any modern browser. |
...
| Shareable results dashboard (URL/QR) | ✅ | ⚠️ AnyLogic cloud sharing | DES Studio's hash-route public dashboards and QR codes are simpler and more portable. |
| Community model gallery + fork-to-run | ✅ | ⚠️ AnyLogic Cloud | DES Studio's fork mechanism is more permissive and discoverable. |
| Real-time data adapters (REST/WebSocket) | ✅ | ✅ AnyLogic | DES Studio's AdapterRegistry pattern is more extensible (registry vs hardcoded). |
| Calibrated batch + rolling run modes | ✅ | ❌ None | Unique to DES Studio's adapter architecture. |
| Plan vs actual deviation metric | ✅ | ❌ None | Unique: avgPlanDeviation, ActualsStreamAdapter. |
| Seeded reproducibility contract | ✅ | ⚠️ Several tools | DES Studio enforces seeded PRNG as a formal contract; most tools rely on Math.random(). |
...
| Model versioning with structural change detection | ✅ | ⚠️ Simio branching | DES Studio categorises changes as structural vs parameter; most tools version the file, not the model. |

### Where DES Studio Is Competitive (Parity)
...
| **High-dimensional optimisation** | ❌ Behind OptQuest (Simio/Arena) | Practical limit ~4 parameters (combinatorial sweep). | DES Studio's AI-driven goal-directed sweep is more interpretable but cannot handle 10-50 parameter spaces. |
...
DES Studio is the **only tool in the comparison with a formal no-code-injection guarantee**. While all other tools rely on `eval()`, `new Function()`, or user-written code in full programming languages (Python, Java, C#, SIMAN), DES Studio enforces:
...
This is especially important for a multi-user tool where public models could be viewed by any authenticated user. A crafted public model in AnyLogic, SimPy, or Simio could execute arbitrary code in the viewer's environment. DES Studio's architecture prevents this class of attack entirely.

### Reproducibility

DES Studio enforces a formal **seeded reproducibility contract** (Sprint 29) — all six invariants are tested in CI:
...
|------|--------------------|----------|
| DES Studio | ~1000+ (Vitest) | Engine (~120), UI (~200+), DB (~50), templates, benchmarks, CI gates |
...
DES Studio's test count is lower than SimPy's but covers a broader surface (UI + DB + engine) and includes three correctness gates that must pass before any sprint is considered complete.
...
| Professional Gap Analysis (archived) | `docs/archived/des-studio-professional-gap-analysis.md` | Superseded by capability-gap-analysis; historical reference |
...
| Build Plan | `docs/DES_Studio_Build_Plan.md` | Sprint history and roadmap |
