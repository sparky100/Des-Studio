...
| G04 | **Resource breakdowns / failures** | B | **High** | ✅ Complete | `FAIL` and `REPAIR` macros; MTBF/MTTR auto-scheduling. Sprint 32. |
| G05 | ~~**Clock value in conditions**~~ | F | ~~Low~~ | ✅ Resolved | `clock` token in `buildConditionTokens()`. Sprint 31. |
| G06 | ~~**Entity splitting / cloning**~~ | A | ~~Med~~ | ✅ Resolved | `SPLIT` macro with parent-child tracking. Sprint 33. |
...
| G10 | ~~**Entity matching / synchronisation**~~ | C | ~~Med~~ | ✅ Resolved | `MATCH` macro. Sprint 33. |
