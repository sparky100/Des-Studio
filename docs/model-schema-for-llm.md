...
| `exposedParams` | array | No | **App-managed — do NOT generate or modify.** Owner-curated list of parameters that viewer-role users may adjust in the Business view (`{path, businessLabel?, min?, max?}` per entry, where `path` is an internal sweep-parameter path). The app preserves the model's existing value across AI applies. If present in a model you are asked to modify, copy it through unchanged. |
| `skills` | string[] | No | Registry of all skill names used in the model. Each entry is a unique skill name that server entity types can declare as possessed (see §2) and that ASSIGN/COSEIZE macros or condition predicates can reference when matching servers to tasks. **Entity-side skills:** Customer entity types can carry a string attribute (e.g. `requiredSkill`) set via `Categorical` distribution at arrival; the `ASSIGN(Q, Server, Entity.requiredSkill)` form reads the attribute at runtime to determine which skill to require. **Per-instance server skills:** Use `skillProfiles` on a server entity type to assign different skill subsets to individual servers via count-based or weight-based profiles. See §2. Default `[]`. |
...
- **When `skillProfiles` is absent or empty,** all servers share the type-level `skills[]` identically (classic behaviour).

### Per-Instance Server Skills (`skillProfiles`)

Define individual skill assignments for each server in a pool. Profiles are **non-exclusive** — a server can match multiple profiles, and its final skills are the union.

```json
{
  "id": "et_doctor_per_instance",
  "name": "Doctor",
  "role": "server",
  "count": 4,
  "skills": ["Surgery", "Consultation", "Triage"],
  "skillProfiles": [
    { "name": "Surgeon",     "skills": ["Surgery", "Consultation"], "count": 2 },
    { "name": "Consultant",  "skills": ["Consultation"],             "count": 1 },
...
This produces:
| Server | Surgeon (count:2) | Consultant (count:1) | Triage Cert (weight:100) | Result |
|---|---|---|---|---|
| #0 | Yes | No | Yes | Surgery, Consultation, Triage |
| #1 | Yes | No | Yes | Surgery, Consultation, Triage |
| #2 | No | Yes | Yes | Consultation, Triage |
| #3 | No | No | Yes | Triage |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `name` | string | No | Display label. Auto-generated from skills if absent. |
...
| `Piecewise`   | `{ "periods": [{ "startTime": "0", "dist": "Exponential", "distParams": { "mean": "3" } }, ...], "cycleLength": "10080" }` | First period must start at 0 (V12); periods sorted ascending (V13); nested Piecewise not supported (V12). `cycleLength` is optional — if set, the clock wraps `% cycleLength` so the period list repeats (e.g. one week = 10080 minutes) instead of holding the last period's rate forever; must be a positive number (V71) and every period's `startTime` must be less than it (V72). |
...
| `SchedulePattern` | `{ "schedulePattern": { "type": "weekly", "mode": "absolute", "periods": [{ "dayOfWeek": 1, "start": "09:00", "end": "17:00", "capacity": "120" }], "defaultCapacity": "0" } }` | Drives a B-event's (or C-event's) arrival/completion timing from the same weekly 24×7 grid used for server capacity (§ Weekly Schedule Pattern), reinterpreting each period's `capacity` as **arrivals per hour** and compiling it once per build to a `cycleLength`-wrapped Piecewise-Exponential internally. Requires `model.epoch` (V55); `mode: "multiplier"` is not supported for arrivals (V75, blocking); `exceptions` are structurally validated but ignored — the weekly pattern repeats unconditionally, including on exception dates (V54, warning). Shares V50/V51/V53/V54 structural checks with the entity-type capacity pattern. A zero-rate ("closed") period is valid (V74, warning) — no arrivals occur during it; by design, at most one extra arrival can fire at the instant a closed window reopens (a documented v1 approximation, not a bug). |
...
- `idle(ServerType).count` / `busy(ServerType).count` — idle/busy server counts (all servers of the type)
- `idle(ServerType, "Skill").count` / `busy(ServerType, "Skill").count` — idle/busy counts filtered to servers whose `skills[]` array contains the named skill. The skill string is case-sensitive and must be a valid entry in the model's top-level `skills` registry (V-SKILL-2). Example: `"idle(Doctor, 'Surgery').count > 0"`
...
| V71 | Piecewise `cycleLength`, if set, must be a positive number |
| V72 | Piecewise `cycleLength`, if set, every period's `startTime` must be less than it — otherwise that period is never reached |
| V73 | SchedulePattern arrival distribution: period arrival rate (`capacity`) must be a number ≥ 0 |
| V74 (warning) | SchedulePattern arrival distribution: a period's arrival rate is 0 — no arrivals occur during that window |
| V75 | SchedulePattern arrival distribution does not support `mode: "multiplier"` — use absolute arrival-rate values |
