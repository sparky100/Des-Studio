...
- The UI already surfaces DB errors from `saveModel` and `saveSimulationRun`; quota errors propagate as-is with the `quota_exceeded:` prefix in the message
...
- `is_user_suspended(uid)` security-definer function — used in RLS policies and the app to avoid recursive policy evaluation
...
| `src/ui/AdminPanel.jsx` | Suspend/Unsuspend in users table; Audit Log tab; action logging on role/suspend/config changes; imports extended |
| `src/ui/UserSettingsPanel.jsx` | New user settings panel (NEW) |
| `src/App.jsx` | Password reset flow (`handleForgotPassword`, `handlePasswordReset`); `PASSWORD_RECOVERY` event handling; suspended-user screen; Settings button + `UserSettingsPanel` integration |
...
| `docs/reviews/sprint-38-plan.md` | Sprint plan (NEW) |
| `docs/reviews/sprint-38-closure-report.md` | This document (NEW) |
...
- [x] "Forgot password?" flow sends a recovery email and the reset page updates the password successfully
