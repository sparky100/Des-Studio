...
| Duplicate / copy-paste | `Ctrl+D` duplicates the current selection in place via `duplicateVisualNodes(model, nodes, offset)`; `Ctrl+C`/`Ctrl+V` copy to an in-memory clipboard and paste at a cursor-relative offset. Connections are never copied — duplicates land disconnected, same as a freshly added node. Cloned Source/Activity nodes get independent bEvent/cEvent clones (own schedule, own completion event) rather than sharing the original's. |
| Probabilistic-branch probability edit | Selecting an edge derived from a `probabilisticRouting` branch (edge carries `bEventId`/`branchIndex`/`probability`, set in `deriveGraphFromModel`) swaps the read-only `%` label for an inline number input. `updateProbabilisticBranchProbability(model, edge, probability)` clamps to `[0, 1]` and writes back to `bEvents[].probabilisticRouting[branchIndex].probability` only — other branches and the routing target are untouched. The canvas does not enforce branch probabilities summing to 1, matching `BEventEditor`'s own non-blocking total. |
