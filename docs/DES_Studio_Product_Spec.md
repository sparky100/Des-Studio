...
| S17 | **Sign-in Welcome dialog** | Shown once per sign-in session (not on page refresh) to all users. Four options: Create a Model (opens New Model dialog with describe / draw / define choice), Access the Model Library (goes to My Models), Build with AI Tools (downloads AI Prompt Pack), Get Help (opens Help Assistant). Escape / Skip for now dismisses to the library. |
| S18 | **AI Prompt Pack export** | Downloadable Markdown file containing the full `model-schema-for-llm.md` spec plus a starter prompt and import instructions. Available from the Welcome dialog (Build with AI Tools) and the **↓ AI Prompt Pack** button in the Model Library header. Enables users to generate model JSON using external AI tools (Claude, ChatGPT, etc.) and import the result. |
...
- Piecewise, Schedule, and Empirical distributions are fully Category 1; all 22 built-in templates export runnable scripts.
