...
| M5 | **Predicate Builder** | Type-safe, point-and-click condition editor for Conditional Events. No free-text logic. Operator set constrained by attribute type. |
| M6 | **Distribution library** | 14 distribution types: Exponential, Uniform, Normal, Triangular, Fixed, Lognormal, Erlang, Empirical, Categorical, Piecewise, Attribute-based (ServerAttr, EntityAttr), Schedule, Distance (travel time from a declared distance ÷ a speed attribute — Model Data tab). Lognormal is recommended for heavily right-skewed durations (repair times, long-tail task durations). Seeded PRNG (mulberry32). No `Math.random()` in engine. |
...
| M11 | **Define editors** | Structured UI editors for all model element types, accessed via the **Define** button in the Design toolbar. Sub-sections: Entity Types, Queues, Bound Events, Conditional Events, Schedules, Model Data. No free-text logic fields. |
...
| # | Feature | Description |
|---|---------|-------------|
| S1 | **Visual Designer** | @xyflow/react drag-and-drop canvas (**Draw** button). Writes to the same `model_json` as the Define editors. Supports mouse and touch-friendly multi-select, group move, and bulk delete for larger diagrams; node duplication and clipboard copy/paste (Ctrl+D / Ctrl+C / Ctrl+V); inline editing of probabilistic-routing branch probabilities directly on the edge label, without leaving the canvas; selection-based align/distribute tools; and Figma-style alignment guides while dragging a single node — dashed guide lines appear when the dragged node's edges or centre line up with another node's, and the node snaps into alignment on release. |
...
| S4 | **Parametric sweep** | 1D and 2D sweeps with Goal Feasibility line. Two-scenario comparison uses paired-t confidence intervals with Bonferroni correction. tukeyHSD() and oneWayANOVA() are wired to the UI via the named-scenario manager (S25) — comparing 3+ scenario groups renders a one-way ANOVA significance banner plus Tukey HSD pairwise table. |
...
| S17 | **Run admission tier system** | Per-user run tier (Free/Standard/Pro) gates replications, run duration, planned schedule rows, and estimated Conditional event scans before a run starts, derived from a pre-run complexity estimate (the **Workload Estimate** panel on the Execute screen: planned arrivals, expected entities, stage moves, Conditional event scans, confidence). The panel auto-expands whenever the estimate produces an actionable warning or confirmation (e.g. chart data or live trace will be auto-disabled for this run); it can still be manually collapsed afterwards. After a run completes, the panel also shows how the estimate compared to what actually happened (scans/entities ratio vs. estimate), so the estimator's accuracy can be tracked over time. Enforced before each run. |
...
| S21 | **Purge period** | Scheduled queue sweep that removes entities exceeding a maximum dwell time. Configured as a Bound event with a fixed or periodic interval. Complements entity-driven reneging with a global timer-based clearance — useful for batch/job queues operating under SLA deadlines. |
...
| S25 | **Named-scenario manager** | **Named Scenarios** panel on the model Overview tab. A scenario is a named set of parameter deltas against the current model (same sweep-parameter machinery as S4), stored in a dedicated `scenarios` table (RLS scoped to creator) with optional seed and replication count. Selected scenarios — optionally including a "Base model" sentinel — run in-memory on demand and compare via paired-t (2 groups) or one-way ANOVA + Tukey HSD (3+ groups, the AnovaTukeyTable component). Comparison runs are not persisted to run history. |
| S26 | **Business view (viewer-role surface)** | Simplified explore-and-run page shown to users granted `viewer` access, instead of the modelling environment. The owner curates which parameters are exposed (`exposedParams` in `model_json`: path + optional business label + optional min/max) from the Access tab's Business view section. Viewers see the model name/description, the exposed settings as bounded number inputs, and a Run button: replications execute in the viewer's browser from the owner's experiment defaults (capped by the viewer's tier), against a temporary in-memory copy, and results render as the standard KPI summary cards. Orphaned exposed settings (target renamed/removed) are flagged to the owner and hidden from viewers. |
...
| "C-scan restart limit exceeded" | "The simulation detected a possible loop in your service conditions (Conditional Event restart limit reached)" |
...
| Business view runs not persisted | By design (v1) | Viewer runs in the Business view (S26) execute in-memory only — nothing is written to `simulation_runs`, so the owner has no visibility into what viewers tried. A shared results space and an animated watch-mode are noted follow-ups. |
| Editor-role writes blocked by RLS | Known gap | The `des_models` UPDATE policy is owner-scoped, so a granted `editor` gets the full editing UI but their Save fails at the database. Fixing requires an RLS policy change; tracked, not yet scheduled. |
